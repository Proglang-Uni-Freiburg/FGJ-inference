...
def FJType(Pi: FGJ.Pi, class_def: FGJ.ClassDef, CT: FGJ.ClassTable) -> tuple[FGJ_GT.lambdas, FGJ_GT.C]:
    ...
    for method_def in class_def.methods.values():
        ...
        if method_sign:
            ...
            C0e |= hSet([FGJ_GT.SubTypeC(am, method_sign.return_type)])
        else:
            ...
            Cm |= hSet([FGJ_GT.SubTypeC(am, FGJ.NonTypeVar("Object", []))]) | hSet([FGJ_GT.SubTypeC(ai, FGJ.NonTypeVar("Object", [])) for ai in ass])
    BigPi = Pi | l0s | ls
    constraints = C0e | Cm
    for method_def in class_def.methods.values():
...
def TypeMethod(Pi: FGJ.Pi, class_header: FGJ.ClassHeader, method_def: FGJ.MethodDef, CT: FGJ.ClassTable) -> FGJ_GT.C:
    ...
    return Ce | hSet([FGJ_GT.SubTypeC(Re, method_sign.return_type)])
    ...
    match expr:
        ...
        case FGJ.FieldLookup(e0, name):
            ...
            for class_def in CT.values():
                ...
                c_new: FGJ_GT.C = hSet([FGJ_GT.SubTypeC(Re, FGJ.NonTypeVar(class_def.name, a1s)),
                                        FGJ_GT.EqualC(a, AUX.sub(a1s, xs, class_def.typed_fields[name])),
                                        ])
                for ai, ni in zip(a1s, class_def.generic_type_annotation.values()):
                    c_new |= hSet([FGJ_GT.SubTypeC(ai, AUX.sub(a1s, xs, ni))])
                oc |= hSet([c_new])
            return a, Cr | hSet([oc])
            ...
            for elem in Pi.items():
                match elem:
                    ...
                    case ((class_header, method_name), set_of_method_sign):
                        ...
                        oc |= hSet([hSet([FGJ_GT.SubTypeC(Re, FGJ.NonTypeVar(class_header.class_name, ass))]) |
                                    hSet([FGJ_GT.EqualC(a, AUX.sub(bs, ys, AUX.sub(ass, xs, t)))]) |
                                    hSet([FGJ_GT.SubTypeC(Ri, AUX.sub(bs, ys, AUX.sub(ass, xs, ti))) for Ri, ti in zip(Ri, ts)]) |
                                    hSet([FGJ_GT.SubTypeC(bi, AUX.sub(bs, ys, AUX.sub(ass, xs, pi))) for bi, pi in zip(bs, ps)]) |
                                    hSet([FGJ_GT.SubTypeC(ai, AUX.sub(ass, xs, ni)) for ai, ni in zip(ass, ns)])
                                    ])
            c_ret = Cr | hSet([oc])
            for ci in Ci:
                c_ret |= ci
        ...
        case FGJ.NewClass(type, parameters):
            ...
            sc = hSet([FGJ_GT.SubTypeC(Ri, ti) for Ri, ti in zip(RiCi.keys(), typed_fields.values())]) | hSet([FGJ_GT.SubTypeC(ai, AUX.sub(ass, xs, ni)) for ai, ni in zip(ass, ns)])
