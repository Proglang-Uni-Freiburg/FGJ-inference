...
def TypeInference(Pi: FGJ.Pi, index: int, env: FGJ.Delta, CT: FGJ.ClassTable) -> FGJ.Pi:
    print(index)
    ...
    while index >= 0:
        ...
        unifys = [(sig, ysEps) for sig, ysEps in Unify(constraint, dict(class_def.generic_type_annotation.items()) | env, CT)]  # constraint solving
        for sig, ysEps in unifys:
            try:
                ...
                new_pi = TypeInference(Pi | {class_header_method_tuple: [FGJ.MethodSign(getTypeSigOf(method_sign, ysEps, sig, env), [sig[ai] for ai in method_sign.types_of_arguments], sig[method_sign.return_type]) for method_sign in method_signs] for class_header_method_tuple, method_signs in ls.items() if method_signs not in Pi.values()}, index + 1, env | ysEps, CT)
                return new_pi
...
d = TypeInference(dict(), 0, dict(), program.CT)
