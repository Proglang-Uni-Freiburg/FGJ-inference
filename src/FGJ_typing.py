...
def typing_expression(expr: FGJ.Expression, Pi: FGJ.Pi, Delta: FGJ.Delta, Env: FGJ.Env, CT: FGJ.ClassTable) -> FGJ.Type:
    match expr:
        ...
        case FGJ.FieldLookup(e0, field_name):
            t0 = typing_expression(e0, Pi, Delta, Env)
        ...
        case FGJ.NewClass(type, parameters):
            n: FGJ.Type = ...  # asumes some us for N = C<us> (C == type.name)
...
def typing_method(method_def: FGJ.MethodDef, class_def: FGJ.ClassDef, Pi: FGJ.Pi, Delta: FGJ.Delta, CT: FGJ.ClassTable):
    ...
    for method_sign in Pi[(class_header, method_def.name)]:
        ...
        for x, ti in zip(method_def.typed_parameters.keys(), ts):
            Env[x] = ti
        s = typing_expression(e0, Pi, Delta, Env)
...
def typing_class(class_def: FGJ.ClassDef, Pi: FGJ.Pi, CT: FGJ.ClassTable):
    ysEps: FGJ.GenTypeAno = ...  # infered
    Pi1 = Pi | {(FGJ.ClassHeader(class_def.name, class_def.generic_type_annotation), method_def.name): {FGJ.MethodSign(dict(), method_def.typed_parameters.values(), method_def.return_type)} for method_def in class_def.methods.values()}
    Pi2 = Pi | {(FGJ.ClassHeader(class_def.name, ysEps), method_def.name): {FGJ.MethodSign(method_def.generic_type_annotation, method_def.typed_parameters.values(), method_def.return_type)} for method_def in class_def.methods.values()}
